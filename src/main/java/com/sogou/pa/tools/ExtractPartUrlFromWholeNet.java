...
public class ExtractPartUrlFromWholeNet extends Configured implements Tool {
	
	public static void main(String[] args) throws Exception
	{
		ToolRunner.run(new ExtractPartUrlFromWholeNet(), args);
	}
	...
	public int run(String[] args) throws Exception {
		try {
			...
			job.setJarByClass(ExtractPartUrlFromWholeNet.class);
	...
	public static class Map extends Mapper<Object, Text, Text, Text>
	{
		...
		{ 
			...
			if(tmp.length>=24)
			{
				...
				{
					context.write(new Text(tmp[0].split("\\s")[0]+"|"+tmp[23]), new Text("0"));
				}
			}
		}
	}
